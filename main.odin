...
/*
colorFromFloat :: proc(val, max:f32)->rl.Color{
	quantized :u8= abs
}*/
...
main :: proc(){
	...
	lambdaPI :f32=math.PI/wavelength //PI/wavelength 
	...
	offsetV2 :rl.Vector2= {0, wavelength*inter_source_dist} //offset in vector2
	offsetf32 :f32= wavelength*inter_source_dist //vertical offset only
	offsetf32_square :f32= offsetf32*offsetf32
	source_pos :[nb_sources]rl.Vector2
	source_pos[0] = {margin, f32(window_height/2)-array_length/2}
	first_source_Y :f32= f32(window_height/2)-array_length/2

	for i:int=1; i<nb_sources; i+=1{
		source_pos[i] = source_pos[i-1]+offsetV2
	}

		//generic utils
	change : bool = true // set to true to update the image
		//for the slider
	...
	for !rl.WindowShouldClose(){
		...
		if change{//there was a change of params since last frame, or this is the first frame
			...
			loop_start :time.Tick = time.tick_now()
			for y :i32=0; y<window_height; y+=1{
				row_Vdist :f32=f32(y)-first_source_Y
				for x :i32=0; x<window_width; x+=1{
					current_pos :rl.Vector2= {f32(x), f32(y)}
					val :f32= 0
					Hdist :f32= f32(x)-margin
					Hdist2 := Hdist*Hdist
					Vdist :f32 = row_Vdist

					for idx :=0; idx<nb_sources; idx +=1{
						val +=math.sin_f32(math.sqrt_f32(Hdist2 + Vdist*Vdist)*lambdaPI+phi*f32(idx))
							Vdist -= offsetf32
					}
			...
			}fmt.println("looping the image took: ", time.tick_since(loop_start))
		...
		if rl.IsMouseButtonReleased(.LEFT) && interacting_with_slider{
